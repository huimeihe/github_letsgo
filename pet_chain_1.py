...
class PetChain():
	...
	def purchase(self, pet):
		try:
			...
			if (float(pet_amount) <= float(self.degree_conf.get(pet_degree))   )\
				or (float(pet_amount)<= float(self.buyprice[pet_degree][0])   and(  self.inventory[pet_degree][0]<self.inventory[pet_degree][2]  )):
				...
				if pet_degree==3 and rareCounts==4:
					# print("品级低一级，降价购买")
					if (float(pet_amount) >= float(self.degree_conf.get(pet_degree))   )\
						and (  float(pet_amount)>= float(0.3*self.buyprice[pet_degree][0])  ) :
						print("降价购买，价格太高，不买了，原限价{}，现限价{}".format(self.buyprice[pet_degree][0],0.3*self.buyprice[pet_degree][0]))
						return
				'''
				if pet_degree==4 and rareCounts==6:
					# print("品级低一级，降价购买")
					if (float(pet_amount) >= float(self.degree_conf.get(pet_degree))   )\
						and (  float(pet_amount)>= float(0.25*self.buyprice[pet_degree][0])  ) :
						print("降价购买，价格太高，不买了，原限价{}，现限价{}".format(self.buyprice[pet_degree][0],0.25*self.buyprice[pet_degree][0]))
						return				
				'''
	...
	def analyze_detail(self,attributes):
		...
		if attributes_dict[u'眼睛']==u'白眉斗眼':
			...
			sellratio=0.25	
		if attributes_dict[u'体型']!=u'天使' and attributes_dict[u'眼睛']!=u'白眉斗眼':
			print('非天使&白眉',end=' ')			
			shouldbuy=True
			sellratio=0.14	
		...
		print('rareCounts',rareCounts,'sellratio',round(sellratio,2))
	...
	def sell_dog(self,pet):
		...
		try:
			...
			sellprice_=round(self.sellprice[rareDegree][0]*sellratio*float(random.uniform(1, 1.3)),0)
			# print(sellprice_)
			data = {
				"petId":petId,
				"requestId": int(time.time() * 1000),
			...
			resp = page.json()			
			if resp.get(u"errorMsg") == u"success":
				print('put on shelf:','rareDegree',rareDegree,'amount',sellprice_,"petId",petId)
			else:
				print("petId",petId,resp.get(u"errorMsg"),resp.get(u"errorNo"))
		except Exception as e:
			print(e) 
...
def analyze_attribute_premium():
	inputfilepath='record_letsgogold/v3_{}.csv'.format((datetime.datetime.now()-datetime.timedelta(hours=0)).strftime( '%Y-%m-%d_%H' ))
	df=pd.read_csv(inputfilepath,  sep='\t')
	...
	# 	print(label)
	# 	plt.figure(i-1)
	# 	g = sns.FacetGrid(df, col=label, col_wrap=1, )#size=None
	# 	g = g.map(plt.hist, "amount", bins=range(0,210000,2000), color="m")		
	# 	plt.show()			
